...
def padding_des_sequences(index_sequences, max_sentnum, max_sentlen, post_padding=True):
    X = np.empty([len(index_sequences), max_sentnum, max_sentlen], dtype=np.int32)
    mask = np.zeros([len(index_sequences), max_sentnum, max_sentlen], dtype=theano.config.floatX)
...
def get_random_batch(batch_size, X_train, Y_train, f):
    batch_X = []
    batch_Y = []
    batch_f = []
    for _ in range(batch_size):
        index = random.randrange(X_train.shape[0])
        batch_X.append(X_train[index])
        batch_Y.append(Y_train[index])
        batch_f.append(f[index])
    return batch_X, batch_Y, batch_f

def get_random_batch1(batch_size, X_train, Y_train):
    batch_X = []
    batch_Y = []
    for _ in range(batch_size):
        index = random.randrange(X_train.shape[0])
...
def get_order_batch(batch_size, X, Y, order_count):
    batch_X = []
    batch_Y = []
    if order_count == X.shape[0]:
        return False
    else:
        start = order_count
        if order_count + batch_size > X.shape[0]:
            end = X.shape[0]
        else:
            end = order_count + batch_size
        for index in range(start, end):
            batch_X.append(X[index])
            batch_Y.append(Y[index])
        order_count = end
    return batch_X, batch_Y, order_count